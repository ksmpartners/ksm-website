/**
 * Returns true when draft content should be hidden (production, or dev with VITE_HIDE_DRAFTS_IN_DEV=true).
 */
export function shouldHideDrafts(): boolean {
  return (
    import.meta.env.PROD ||
    process.env.VITE_HIDE_DRAFTS_IN_DEV === "true"
  );
}
...
function handleDraftPage(pageData: any): Response | undefined {
  if (pageData.draft && shouldHideDrafts()) {
