...
describe("getLocaleUrlCTM", () => {
  ...
  test("Handles absolute URLs and preserves anchors", () => {
    const url = "http://localhost:4321/en/contact/#example-anchor";
    const result = getLocaleUrlCTM(url, "en", prependValue);
    expect(result).toBe(
      "http://localhost:4321/case-studies/contact/#example-anchor",
    );
  });
  ...
  test("Prepends optional value correctly", () => {
    ...
    expect(result).toBe("/es/case-studies/contact/");
  });
